function [x, GA, MA] = topology_optimization( ...
    ...
    while (volume_fraction_error > 0.004 || gray_index > 0.002 || change > 0.01) && (iteration_number < max_iteration_number)
        ...
        sK = reshape( ...
        k_e(:) * (E_min + x(:)' .^ penalization * (E_0 - E_min)), ...
            64 * nel_x * nel_y, ...
            1 ...
        );
        K = sparse(iK, jK, sK);
        ...
        if is_spring
            ...
            elements_objective_function_value = reshape( ...
                sum((U_1(elements_number) * k_e) .* U_2(elements_number), 2), ...
                nel_y, ...
                nel_x ...
            );
            ...
            objective_function_value = sum(sum( ...
                (E_min + x .^ penalization * (E_0 - E_min)) .* elements_objective_function_value ...
            ));
        end
        ...
        sensitivity_objective_function_value = penalization * (E_0 - E_min) * x .^ (penalization - 1) .* elements_objective_function_value;
        sensitivity_volume_fraction = ones(nel_y, nel_x);

        [sensitivity_objective_function_value, sensitivity_volume_fraction] = sensitivity_filter( ...
            x, ...
        ...
        elseif update_method == "mma"
            ...
            if is_spring
                constraint_function_value = [
                    scale * (objective_function_value + 10);
                    sum(x(:) / ((volume_fraction + 0.001) * variable_number)) - 1;
                    -sum(x(:) / ((volume_fraction - max(0, 250 - iteration_number) * 0.001) * variable_number)) + 1;
                ];

                df_i_dx = [
                    scale * sensitivity_objective_function_value(:)';
                    ones(1, nel_y * nel_x) / ((volume_fraction + 0.001) * variable_number);
                    -ones(1, nel_y * nel_x) / ((volume_fraction - max(0, 250 - iteration_number) * 0.001) * variable_number);
                ];

                
                sensitivity_objective_function_value = zeros(variable_number, 1);
            else
                constraint_function_value = sum(x(:) / (volume_fraction * variable_number)) - 1;
                df_i_dx = sensitivity_volume_fraction(:)' / (volume_fraction * variable_number);
            end

            [x_new, ~, ~, ~, ~, ~, ~, ~, ~, asymptote_lower_bound, asymptote_upper_bound] = mma_reference( ...
                constraint_number, ...
                variable_number, ...
                iteration_number, ...
                x_no_project(:), ...
